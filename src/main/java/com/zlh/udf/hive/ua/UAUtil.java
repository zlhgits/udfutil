...
public class UAUtil {
    ...
    public static String getBrand(String ua){
        String bra = "未识别";
        try {
            if(ua.contains("ARM")||ua.contains("NOKIA")||ua.contains("IEMobile")){
                bra = "诺基亚";   //品牌
...
//                                iend = ua.indexOf("like");
    ...
    public static String getLanguage(String ua){
        String langu = "未识别";
        int iret = 0;
        try {
            // 判断语言
    ...
    private static String getBra(String ua) {
        String bra = "未识别";
        for (int i = 0; i < arrPhoneModel.length; i++) {
            String arrmd[] = arrPhoneModel[i].split(",");
            for (int j = 0; j < arrmd.length; j++) {
                int iret = ua.toLowerCase().indexOf(arrmd[j].toLowerCase());
                if (iret >= 0) {
                    bra = arrBrandCN[i];
                    break;
                }
